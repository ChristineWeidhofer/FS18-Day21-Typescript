...
for(let i: number = 0; i < boxs.length; i++){
  ...
  function calcPrice() {
    let newPrice:any = (10000000 / (allVehicles[i].year)).toFixed(2); // toFixed() returns a string!!!
...
const jedi: string[] = ['Obi-Wan', 'Yoda'];
const [x, y] = jedi;
...
function myFunction(a: string, b: string, c: string = "Obi-Wan") {
  console.log(`${a}, ${b}, ${c}.`);
}
